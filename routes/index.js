    ...
    db.query(
      ...
      (err, result) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ error: "خطا در ثبت تصویر در پایگاه داده" });
        }
...
router.get("/get/site/setting", authentication, async (req, res) => {
  const user = await authorization(req);
...
router.post('/update/site/setting', authentication , async (req , res) => {
    const user = await authorization(req);
    const canAccess = await checkPermission(
      user.roles,
